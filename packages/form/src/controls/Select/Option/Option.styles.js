...
export const OptionItem = styled('button')`
  ...
  background-color: ${(p) =>
    p.focused ? getTheme(p.theme, 'colors.lighterPrimary') : getTheme(p.theme, 'colors.white')};
  font-size: 13px;
  font-weight: normal;
  font-style: normal;
  font-stretch: normal;
...
export const IconWrapper = styled('div', { shouldForwardProp: (prop) => !['color'].includes(prop) })`
  font-size: 24px;
  color: ${(p) => p.color};
  display: inline-flex;
  margin-left: 8px;
